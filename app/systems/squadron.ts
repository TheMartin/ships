...
export class Squadron implements NetworkComponent
{
  ...
  {
    return this.flagship === other.flagship;
  }

  clone() : Squadron
  {
    return new Squadron(this.flagship);
  }
...
export class SquadronMember implements NetworkComponent
{
  ...
  {
    return this.squadron === other.squadron
      && ((this.offset === null && other.offset === null) || (this.offset !== null && other.offset !== null && this.offset.equal(other.offset)));
  }

  clone() : SquadronMember
  {
    return new SquadronMember(this.squadron, this.offset ? this.offset.clone() : null);
  }

  serialize() : any[]
  {
    return [ this.squadron, ...(this.offset ? [ this.offset.x, this.offset.y ] : [ null, null ]) ];
  }
    ...
    world.forEachEntity([Squadron], (id : Entity, components : any[]) =>
    {
      ...
      if (!world.containsEntity(squadron.flagship))
      {
        ...
        if (members.length > 0)
        {
          ...
          {
            let leadMember = world.getComponent(members[0], SquadronMember) as SquadronMember;
            let offset = leadMember.offset;
            leadMember.offset = null;
            for (let i = 1; i < members.length; ++i)
            {
              (world.getComponent(members[i], SquadronMember) as SquadronMember).offset.subtract(offset);
            }
          });
        }
        else
...
export class SquadronMovement implements System
{
  ...
  update(dt : number, world : World, deferred : Deferred)
  {
    world.forEachEntity([Position, Rotation, Velocity, AngularVelocity, SquadronMember], (id : Entity, components : any[]) =>
    {
      ...
      let [flagshipPosition, flagshipRotation] = world.getComponents(squadron.flagship, [Position, Rotation]) as [Position, Rotation];
